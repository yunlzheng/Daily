angular.module('ionicDaily', ['ionic'])
    ...
    .factory('Dailys', function($http, Daily) {
        ...
        return {
            ...
            fetchLatest: function() {
                return Daily.fetchLatest();
            },
            fetchBeforeDay: function(dayStrs) {
                return $http.get('http://daily.zhihu.com/api/2/news/before/' + dayStrs);
            },
            fetchStory: function(id) {
                return Daily.fetchStory(id);
            }
        }

    }) 

    .controller('NewsCtrl', function($scope, $timeout, Daily, Dailys) {
        ...
        function fetchLatest() {
            Daily.fetchLatest().success(function(data) {
                $scope.dailys.push(data.news);
                Dailys.saveAll($scope.dailys);
                refrshComplete();
            }).error(function() {
